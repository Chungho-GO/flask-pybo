...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect('{0}#comment_{1}'.format(url_for('question.detail', question_id=question_id),comment.id))
...
def modify_question(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            return redirect('{0}#comment_{1}'.format(url_for('question.detail', question_id=comment.question.id),comment.id))
    else:
